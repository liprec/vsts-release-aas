...
function Get-AzureActiverDirectoryResourceId {
    ...
    if (($endpoint.Data.Environment) -and ($endpoint.Data.Environment -eq $azureStack)) {
        ...
        if ($null -ne $activeDirectoryResourceid) {
            if ($activeDirectoryResourceid[-1] -ne '/') {
                $activeDirectoryResourceid += "/"
            }
            $activeDirectoryResourceid += ".default"
        }
    }
...
function Get-UsernamePasswordAccessToken {
    ...
    $authUri = "$authUrl/common/oauth2/v2.0/token"
    $body = @{
        scope = $script:azureUri
    ...
    Write-Verbose "Fetching Access Token (usernamepassword)"
...
function Get-SpnAccessToken {
    ...
    $method = "POST"
    $authUri = "${envAuthUrl}${tenantId}/oauth2/v2.0/token"
    $body = @{
        scope = $azureActiveDirectoryResourceId
    ...
    # Call Rest API to fetch AccessToken
    Write-Verbose "Fetching Access Token (spn)"
...
Export-ModuleMember -Function Get-ExceptionMessage
Export-ModuleMember -Function Get-ConnectionType
Export-ModuleMember -Function IsAzureRmConnection